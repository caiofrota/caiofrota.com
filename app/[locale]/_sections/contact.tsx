...
import { ArrowRight, Github, Instagram, Linkedin, LoaderCircle, type LucideIcon } from "lucide-react";
...
export function Contact() {
  ...
  return (
    <section id="contact" className="mx-auto max-w-7xl scroll-mt-20 px-5 py-24 sm:px-8 md:py-32 lg:px-10">
      <Reveal>
        <SectionHeading kicker={t.contact.kicker} title={t.contact.title} subtitle={t.contact.subtitle} />
      </Reveal>
      <div className="grid gap-6 md:grid-cols-2">
        <Reveal className="h-full" index={0} stagger={90}>
            ...
            <form onSubmit={handleSubmit} className="relative grid gap-4">
              ...
              <label className="text-sm">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-sm text-slate-900 transition-[border-color,box-shadow,background-color] duration-200 placeholder:text-neutral-400 dark:border-slate-700 dark:bg-slate-800 dark:text-neutral-100 cf-ring"
              ...
              <label className="text-sm">
                ...
                <textarea
                  ...
                  className="w-full resize-none rounded-xl border border-neutral-300 bg-white px-3 py-2 text-sm text-slate-900 transition-[border-color,box-shadow,background-color] duration-200 placeholder:text-neutral-400 dark:border-slate-700 dark:bg-slate-800 dark:text-neutral-100 cf-ring"
              ...
              <div aria-live="polite">
                {errorMessage ? (
                  <p role="alert" className="status-message text-sm text-red-700 dark:text-red-300">
                    {errorMessage}
                  </p>
                ) : null}
                {sentSuccess ? (
                  <p role="status" className="status-message text-sm text-emerald-700 dark:text-emerald-300">
                    {t.contact.form.success}
                  </p>
                ) : null}
              </div>
              <button
                className="group inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-linear-to-r cf-accent px-5 py-3 text-sm font-semibold text-white shadow-xl transition-[transform,box-shadow,opacity] duration-200 hover:-translate-y-0.5 hover:shadow-2xl active:translate-y-0 active:scale-[.98] motion-reduce:transform-none disabled:cursor-wait disabled:opacity-50 disabled:shadow-none"
                type="submit"
        ...
        <Reveal className="h-full" index={1} stagger={90}>
          <Card className="h-full">
              ...
              {SOCIAL_LINKS.map((social) => {
                ...
                return (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="group inline-flex items-center justify-between rounded-2xl border border-neutral-200 px-4 py-3 text-sm transition-[transform,background-color,border-color] duration-200 hover:translate-x-1 hover:bg-neutral-50 motion-reduce:transform-none dark:border-slate-700 dark:hover:bg-slate-700/50"
                    title={social.label}
                  >
                    ...
                    <ArrowRight className="h-4 w-4 translate-x-0 opacity-0 transition-[opacity,transform] duration-200 group-hover:translate-x-1 group-hover:opacity-100 motion-reduce:transform-none" />
                  </a>
                );
              })}
            </div>
