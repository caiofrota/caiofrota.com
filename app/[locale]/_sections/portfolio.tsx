...
export function Portfolio({ locale }: { locale: string }) {
  ...
  return (
    <section id="projects" className="mx-auto max-w-7xl px-5 py-24 sm:px-8 md:py-32 lg:px-10">
      ...
      <div className="mt-14 grid gap-6 lg:grid-cols-2 xl:grid-cols-3">
        {content.cases.map((item, index) => (
          <article
            key={item.title}
            className="group flex min-h-full flex-col rounded-3xl border border-slate-200/80 bg-white/55 p-7 transition hover:-translate-y-1 hover:border-cyan-600/35 hover:bg-white/85 dark:border-slate-700/55 dark:bg-slate-800/35 dark:hover:border-cyan-400/35 dark:hover:bg-slate-800/60 sm:p-8"
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <ArrowUpRight className="size-5 text-slate-500 transition group-hover:text-cyan-700 dark:text-slate-500 dark:group-hover:text-cyan-300" />
            </div>
