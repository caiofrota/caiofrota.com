...
type SocialType = "github" | "instagram" | "linkedin";
type Props = {
    ...
    <div id="footer-socials" className="flex items-center gap-4">
      {socials.map(({ type, url }) => {
        ...
        return (
          ...
          >
            <Icon /> {showLabels && CONFIG[type].label}
          </Link>
        );
      })}
    </div>
...
const CONFIG: Record<SocialType, { label: string; icon: React.ComponentType<React.SVGProps<SVGSVGElement>> }> = {
